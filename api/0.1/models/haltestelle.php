...
class Haltestelle extends Databaserelation {
  ...
  function findByRadius($lat, $lng, $radius) {
	...
	$sql = "SELECT oid, standort, buslinien, Y(the_geom) AS latitude, X(the_geom) AS longitude, ST_distance(st_transform(GeometryFromText('POINT(".$lng." ".$lat.")',4326),25833),transform(the_geom,25833)) AS distance FROM ".$this->dbschema.".bushaltestellen WHERE ST_distance(st_transform(GeometryFromText('POINT(".$lng." ".$lat.")',4326),25833),transform(the_geom,25833)) < ".$radius_in_meter." ORDER BY distance LIMIT 10";
