...
class ddl {
		...
		for ($j = 0; $j < count($this->layout['texts']); $j++) {
			...
			if (in_array($this->layout['texts'][$j]['id'], $this->remaining_freetexts) AND $this->layout['texts'][$j]['posy'] != '') {	# nur Freitexte mit einem y-Wert werden geschrieben
				if (
					(
						$type == 'fixed' AND
						$this->layout['texts'][$j]['type'] != 2 AND
						(
					...
					(
						...
						$this->layout['texts'][$j]['type'] == 0
					) OR
					(
						$type == 'everypage' AND
						$this->layout['texts'][$j]['type'] == 2
					)
				) {
					...
					if ($offset_attribute != '') { # ist ein offset_attribute gesetzt
						...
						if ($offset_value != '') {
							# dieses Attribut wurde auch schon geschrieben, d.h. dessen y-Position ist bekannt -> Freitext relativ dazu setzen 
					...
					if ($type == 'running') { # fortlaufende Freitexte
						...
						if ($offset_attribute == '' AND $this->i_on_page > 0) {
					...
					if (
						!$this->miny[$this->pdf->currentContents] OR
	...
	function add_lines($offsetx, $type) {
		...
		for ($j = 0; $j < count($this->layout['lines']); $j++) {
			if ($type != 'everypage' AND $this->page_overflow) {
				$this->pdf->reopenObject($this->page_id_before_sublayout);
			...
			if (
				in_array($this->layout['lines'][$j]['id'], $this->remaining_lines) AND
				$this->layout['lines'][$j]['posy'] != ''
			) {
				# nur Linien mit einem y-Wert werden geschrieben
				if (
					(
						$type == 'fixed' AND
						$this->layout['lines'][$j]['type'] != 2 AND
						(
							$this->layout['type'] == 0 OR
	...
	function add_attribute_elements($selected_layer_id, $layerdb, $attributes, $offsetx, $i, $preview) {
		for ($j = 0; $j < count($attributes['name']); $j++) {
			...
			if (
						...
						case 'SubFormPK' : case 'SubFormEmbeddedPK' : {
							if ($this->layout['elements'][$attributes['name'][$j]]['font'] != '') {
								...
								if ($preview) {
									...
									for ($p = 0; $p < count($attributes['name']); $p++) {
										# erstmal alle Suchparameter des übergeordneten Layers für die Layersuche leeren
									...
									for ($p = 0; $p < count($attributes['subform_pkeys'][$j]); $p++) {
										# die Suchparameter für die Layersuche
									...
									for ($p = 0; $p < count($attributes['subform_pkeys'][$j]); $p++) {
										# die Suchparameter für die Layersuche wieder leeren
						...
						default : {
							...
							if (
								...
								if ($attributes['form_element_type'][$j] == 'Dokument') {
									...
									if (
										$dateiname == $this->attributes['alias'][$j] AND
										$preview
									) {
										#als Platzhalter im Editor
										$dateiname = WWWROOT.APPLVERSION.GRAPHICSPATH.'nogeom.png';
									}
				...
				elseif ($attributes['name'][$j] == $attributes['the_geom'] AND $this->layout['elements'][$attributes['name'][$j]]['xpos'] > 0) {
					# Geometrie
	...
	function handlePageOverflow($offset_attribute, $offset_value, $ypos) {
		...
		if ($offset_value - $ypos < 40) {
			...
			if ($next_page != NULL) {
				...
				# wenns noch keine gibt, neue Seite erstellen
	...
	function getNextPage($pageid) {
		...
		for ($i = 0; $i <= count($pages); $i++) {
			if ($pages[$i]+1 == $pageid AND $pages[$i+1] != '') {
				# die Page-IDs sind komischerweise alle um 1 größer
				return $pages[$i+1]+1;
			}
		}
		return NULL;
	...
	function putText($text, $fontsize, $width, $x, $y, $offsetx) {
		...
		if ($page_id_before_puttext != $page_id_after_puttext) {
			...
			if ($this->getNextPage($page_id_before_puttext) != $page_id_after_puttext) {
				# eine oder mehr Seiten übersprungen -> Fehler
				$this->pdf->overflow_error = true;
			}
		}
		return $ret;
	}
	...
	function get_result_value_output($i, $j, $preview) {
		...
		if ($this->result[$i][$this->attributes['name'][$j]] == '') {
			# wenns der result-value leer ist, ein Leerzeichen setzen, wegen der relativen Positionierung
			$this->result[$i][$this->attributes['name'][$j]] = ' ';
		}
		switch ($this->attributes['form_element_type'][$j]) {
			case 'Auswahlfeld' : {
				if (is_array($this->attributes['dependent_options'][$j])) {
	...
	function createDataPDF($pdfobject, $offsetx, $offsety, $layerdb, $layerset, $attributes, $selected_layer_id, $layout, $result, $stelle, $user, $preview = NULL, $record_paging = NULL) {
		...
		if ($pdfobject == NULL) {
			...
			# ein PDF-Objekt wurde aus einem übergeordneten Druckrahmen/Layer übergeben
			$this->pdf = $pdfobject;
		}
		$this->miny[$this->pdf->currentContents] = 1000000;
		$this->max_dataset_height = 0;
		if ($this->offsety) {
			$this->miny[$this->pdf->currentContents] = 842 - $this->offsety;
		}
		...
		for ($i = 0; $i < count($result); $i++) {
			...
			if (
				...
				if (!$this->initial_yoffset) {
					# der Offset von oben gesehen, mit dem das erste fortlaufende Element auf der ersten Seite beginnt; wird benutzt, um die fortlaufenden Elemente ab der 2. Seite oben beginnen zu lassen
				...
				if ($this->page_overflow != false) {
					if (
						...
						(
							...
							$this->getNextPage($this->transaction_start_pageid) != $this->pdf->currentContents OR
							$this->transaction_start_y > $this->miny[$this->pdf->currentContents] - 50
						)
					) {
						...
						$this->maxy = 0;
						if (!$this->initial_yoffset) {
							# der Offset von oben gesehen, mit dem das erste fortlaufende Element auf der ersten Seite beginnt;
							#wird benutzt, um die fortlaufenden Elemente ab der 2. Seite oben beginnen zu lassen
							$this->initial_yoffset = 780-$this->maxy;
						}
						if ($this->layout['type'] == 2) {
							$this->offsety = $this->pdf->ez['topMargin'];
	...
	function add_everypage_elements() {
		...
		for ($i = 0; $i < $pagecount; $i++) {
			$this->pdf->reopenObject($pages[$i]+1);
			# die Page-IDs sind komischerweise alle um 1 größer
			if ($this->layout['record_paging']) {
		...
		if ($formvars['name']) {
			...
			if ($_files['bgsrc']['name']) {
				...
				if (move_uploaded_file($_files['bgsrc']['tmp_name'], $nachDatei)) {
					$bgsrc = $_files['bgsrc']['name'];
				}
			}
			else {
				$bgsrc = $formvars['bgsrc_save'];
			}
			$sql = "
				INSERT INTO
					`datendrucklayouts`
				SET
					" . ($formvars['gap'] != '' ? "`gap` = " . (int)$formvars['gap'] . "," : "") . "
					`name` 								= '" . $formvars['name'] . "',
					`font_date` 					= '" . $formvars['font_date'] . "',
					`font_user` 					= '" . $formvars['font_user'] . "',
					`layer_id` 						= " . (int)$formvars['selected_layer_id'] . ",
					`margin_top` 					= " . (int)$formvars['margin_top'] . ",
					`margin_bottom` 			= " . (int)$formvars['margin_bottom'] . ",
					`margin_left` 				= " . (int)$formvars['margin_left'] . ",
					`margin_right` 				= " . (int)$formvars['margin_right'] . ",
					`no_record_splitting` = " . (int)$formvars['no_record_splitting'] . ",
					`bgposx` 		= " . ($formvars['bgposx'] ? (int)$formvars['bgposx'] : "NULL") . ",
					`bgposy` 		= " . ($formvars['bgposy'] ? (int)$formvars['bgposy'] : "NULL") . ",
					`bgwidth` 	= " . ($formvars['bgwidth'] ? (int)$formvars['bgwidth'] : "NULL") . ",
					`bgheight` 	= " . ($formvars['bgheight'] ? (int)$formvars['bgheight'] : "NULL") . ",
					`dateposx` 	= " . ($formvars['dateposx'] ? (int)$formvars['dateposx'] : "NULL") . ",
					`dateposy` 	= " . ($formvars['dateposy'] ? (int)$formvars['dateposy'] : "NULL") . ",
					`datesize` 	= " . ($formvars['datesize'] ? (int)$formvars['datesize'] : "NULL") . ",
					`userposx` 	= " . ($formvars['userposx'] ? (int)$formvars['userposx'] : "NULL") . ",
					`userposy` 	= " . ($formvars['userposy'] ? (int)$formvars['userposy'] : "NULL") . ",
					`usersize` 	= " . ($formvars['usersize'] ? (int)$formvars['usersize'] : "NULL") . ",
					`type` 			= " . ($formvars['type'] != '' ? (int)$formvars['type'] : "NULL") . ",
					`filename` 	= " . ($formvars['filename'] ? "'" . $formvars['filename'] . "'" : "NULL") . ",
					`bgsrc` = '" . $bgsrc . "'
			";
			...
			$sql = "
				INSERT INTO
					ddl2stelle (stelle_id, ddl_id)
				VALUES
					( " . $stelle_id . ", " . $lastddl_id . ")
			";
			$this->debug->write("<p>file:kvwmap class:ddl->save_ddl :",4);
			$this->database->execSQL($sql, 4, 1);
			...
			for ($i = 0; $i < count($attributes['name']); $i++) {
				...
				$sql = "
					...
					SET
						...
						font 							= '" . $formvars['font_' . $attributes['name'][$i]] . "',
						ddl_id 						= " . $lastddl_id . ",
						xpos 							= " . (real)$formvars['posx_'.$attributes['name'][$i]] . ",
						ypos 							= " . (real)$formvars['posy_'.$attributes['name'][$i]] . ",
						offset_attribute 	= " . ($formvars['offset_attribute_' . $attributes['name'][$i]]
							? "'" . $formvars['offset_attribute_' . $attributes['name'][$i]] . "'"
							: "NULL"
						) . ",
						width = 					" . ($formvars['width_'.$attributes['name'][$i]]
							? (int)$formvars['width_'.$attributes['name'][$i]]
							: "NULL"
						) . ",
						border = 					" . ($formvars['border_'.$attributes['name'][$i]]
							? (int)$formvars['border_'.$attributes['name'][$i]]
							: "NULL"
						) . ",
						fontsize = 				" . ($formvars['fontsize_'.$attributes['name'][$i]]
							? (int)$formvars['fontsize_'.$attributes['name'][$i]]
							: "NULL"
						) . "
				";
				#echo $sql;
			...
			$sql = "
				...
				WHERE
					(
						...
						(xpos = 0 AND ypos = 0) OR
						(xpos > 595 AND ypos > 842)
					) AND
					ddl_id = " . $lastddl_id . "
			";
			#echo $sql;
			...
			for ($i = 0; $i < count($formvars['text']); $i++) {
				...
				if ($formvars['text'][$i] == 'NULL') {
					$formvars['text'][$i] = NULL;
				}
				if ($formvars['textfont'][$i] == 'NULL') {
					$formvars['textfont'][$i] = NULL;
				}
				$sql = "
					INSERT INTO
						druckfreitexte
					SET
						`text` 							= '" 	. $formvars['text'][$i] . "',
						`font` 							= '" 	. $formvars['textfont'][$i] . "',
						`type` 							= '" 	. $formvars['texttype'][$i] . "',
						`posx` 							= " 	. ($formvars['textposx'][$i] !== NULL 		? (int)$formvars['textposx'][$i] 				: "0") . ",
						`posy` 							= " 	. ($formvars['textposy'][$i] !== NULL 		? (int)$formvars['textposy'][$i] 				: "0") . ",
						`offset_attribute` 	= " 	. ($formvars['textoffset_attribute'][$i] 	? "'" . $formvars['textoffset_attribute'][$i] . "'" : "NULL") . ",
						`angle` 						= " 	. ($formvars['textangle'][$i]							? (int)$formvars['textangle'][$i]       : "NULL") . ",
				";
				#echo $sql;
				$this->debug->write("<p>file:kvwmap class:ddl->save_ddl :",4);
				$this->database->execSQL($sql, 4, 1);
				...
				$sql = "
					INSERT INTO
						ddl2freitexte (ddl_id, freitext_id)
					VALUES
						(" . $lastddl_id . ', ' . $lastfreitext_id . ")
				";
			...
			for ($i = 0; $i < $formvars['linecount']; $i++) {
				$sql = "
					INSERT INTO
						druckfreilinien
					SET
						" . ($formvars['linetype' . $i] == '' ? $formvars['linetype' . $i] . " = 0," : "") . "
						`breite` 	= '" 	. $formvars['breite' . $i] . "',
						`type` 		= '" 	. $formvars['linetype' . $i] . "',
						`posx` 		= " 	. ($formvars['lineposx' . $i] !== NULL ? (int)$formvars['lineposx' . $i] : "NULL") . ",
						`posy` 		= " 	. ($formvars['lineposy' . $i] !== NULL ? (int)$formvars['lineposy' . $i] : "NULL") . ",
						`endposx` = " 	. ($formvars['lineendposx' . $i] !== NULL ? (int)$formvars['lineendposx' . $i] : "NULL") . ",
						`endposy` = " 	. ($formvars['lineendposy' . $i] !== NULL ? (int)$formvars['lineendposy' . $i] : "NULL") . ",
						`offset_attribute_start` = " . ($formvars['lineoffset_attribute_start' . $i] !== NULL
							? "'" . $formvars['lineoffset_attribute_start' . $i] . "'"
							: "NULL"
						) . ",
						`offset_attribute_start` = " . ($formvars['lineoffset_attribute_end' . $i] !== NULL
							? "'" . $formvars['lineoffset_attribute_end' . $i] . "'"
							: "NULL"
						) . "
				";
	...
	function delete_layout($formvars) {
		$sql = "
			DELETE FROM
				`datendrucklayouts`
			WHERE
				id = " . (int)$formvars['selected_layout_id'] . "
		";
		...
		$sql = "
			DELETE FROM
				ddl_elemente
			WHERE
				ddl_id = " . (int)$formvars['selected_layout_id'] . "
		";
		$this->debug->write("<p>file:kvwmap class:ddl->delete_layout :",4);
		$this->database->execSQL($sql, 4, 1);
		...
		$sql = "
			...
			USING
				...
				ddl2freitexte
			WHERE
				ddl2freitexte.freitext_id = druckfreitexte.id AND
				ddl2freitexte.ddl_id = ". (int)$formvars['selected_layout_id'] . "
		";
		$this->debug->write("<p>file:kvwmap class:ddl->delete_layout :",4);
		$this->database->execSQL($sql, 4, 1);
	...
	function load_texts($ddl_id, $freetext_id = NULL) {
		$sql = "
			...
			FROM
				...
				ddl2freitexte
			WHERE
				ddl2freitexte.ddl_id = " . $ddl_id . " AND
				ddl2freitexte.freitext_id = druckfreitexte.id" .
				($freetext_id != NULL ? " AND
				druckfreitexte.id = " . $freetext_id : "") . "
		";
		#echo $sql;
	...
	function addfreetext($formvars) {
		...
		$size = ($formvars['textsize' . $i] != '' ? $formvars['textsize' . $i] 			: 11);
		$font = ($formvars['textfont' . $i] != '' ? $formvars['textfont' . $i] 			: "");
		$posx = ($formvars['textposx' . $i] != '' ? $formvars['textposx' . $i] 			: 70);
		$posy = ($formvars['textposy' . $i] != '' ? $formvars['textposy' . $i] - 20 :  0);
		$sql = "
			INSERT INTO
				druckfreitexte
			SET
		...
		$sql = "
			INSERT INTO
				ddl2freitexte (ddl_id, freitext_id)
			VALUES (" . $formvars['aktivesLayout'] . ", " . $lastinsert_id . ")
		";
		$this->debug->write("<p>file:kvwmap class:ddl->addfreetext :",4);
		$this->database->execSQL($sql, 4, 1);
	...
	function removefreetext($formvars) {
		...
		$sql = "
			DELETE FROM
				ddl2freitexte
			WHERE
				freitext_id = " . $formvars['freitext_id'] . "
		";
		$this->debug->write("<p>file:kvwmap class:ddl->removefreetext :",4);
		$this->database->execSQL($sql, 4, 1);
	}
	...
	function addline($formvars) {
		...
		$breite 	= ($formvars['linebreite' 	. $i] != '' ? $formvars['linebreite' 	. $i]				:   1);
		$posx 		= ($formvars['lineposx' 		. $i] != '' ? $formvars['lineposx' 		. $i] 			:  70);
		$posy 		= ($formvars['lineposy' 		. $i] != '' ? $formvars['lineposy' 		. $i] - 20 	:   0);
		$endposx 	= ($formvars['lineendposx' 	. $i] != '' ? $formvars['lineendposx' . $i] 			: 520);
		$endposy 	= ($formvars['lineendposy' 	. $i] != '' ? $formvars['lineendposy' . $i] - 20 	:   0);
		$sql = "
			INSERT INTO
				druckfreilinien
			SET
		...
		$sql = "
			INSERT INTO
				ddl2freilinien (ddl_id, line_id)
			VALUES
				(" . $formvars['aktivesLayout'] . ", " . $lastinsert_id . ")
		";
		$this->debug->write("<p>file:kvwmap class:ddl->addline :",4);
		$this->database->execSQL($sql, 4, 1);
	}
...
/*
Unformatierte Version zum Vergleich
class ddl {
