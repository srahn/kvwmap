	...
	function parse_path($database, $path, $privileges, $attributes = NULL){
		...
		for($i = 0; $i < $count; $i++){
			...
			if(value_of($privileges, trim($attributename, '"')) != ''){
				...
				if(POSTGRESVERSION >= 930 AND substr($type, 0, 1) == '_' OR is_numeric($type))$newattributesstring .= 'to_json' . (POSTGRESVERSION >= 950? 'b':'') . '('.$real_attributename.') as '.$attributename.', ';		# Array oder Datentyp
