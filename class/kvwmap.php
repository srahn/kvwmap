...
#		if (count($result) == 1) $result = $result[0];
	...
	*/
	...
	function notification_formular() {
		...
		$this->allusers = Nutzer::find($this, 'true');
...
#				echo '<br>cp refmap: ' . IMAGEPATH . basename($refmap) . ' nach: ' . REFERENCEMAPPATH . $refmap_file_name;
			...
			if ($ret[0]) {
				...
				for ($i = 0; $i < count($users); $i++) {
					rolle::create($this->database, $Stelle->id, $users[$i], $Stelle->default_user_id, $layer);
...
//', '', $dump_text));
  ...
  function copyClass($class_id, $layer_id){
	...
	function new_Class($classdata) {
		...
		#echo '<br>sql zum Erzeugen der Klasse: ' . $sql;
	...
	function new_Style($style){
		...
		if (is_array($style)){
			$sql = "
				INSERT INTO styles
				SET
					color = '" . ($style['colorred'] != '' ? $style['colorred'] . " " . $style['colorgreen'] . " " . $style['colorblue'] : $style['color']) . "'
			";
