...
#				$this->Meldung=$ret[1];
  ...
  function wmsExportSenden() {
    ...
    $getMapRequestExample.='&layers='.$this->layerset[0]['alias'];
    for ($i=1; $i < $this->mapDB->anzLayer; $i++){
      $getMapRequestExample.=','.$this->layerset[$i]['alias'];
    }
		...
		if($layer_id != NULL){
			...
			if(!in_array($filetype, array('tiff', 'tif', 'geotif'))){
				switch ($after_import_action) {
					case 'use_geometry' : {
						echo '&nbsp;=>&nbsp;<a href="javascript:void(0);" onclick="javascript:ahah(\'index.php\', \'go=spatial_processing&path1=\'+enclosingForm.pathwkt.value+\'&operation=add_geometry&resulttype=svgwkt&geom_from_layer='.$layer_id.'\', new Array(enclosingForm.result, \'\'), new Array(\'setvalue\', \'execute_function\'));">Geometrie übernehmen</a>';
					}break;
...
//', '', $dump_text));
  ...
  function deleteRollenLayer($id = NULL, $type = NULL){
    ...
    #echo 'SQL: ' . $sql;
		...
		$query=mysql_query($sql);
		if ($query==0) { echo sql_err_msg($PHP_SELF, __LINE__, $sql, $formvars['connection']); return 0; }
  ...
  function read_layer_attributes($layer_id, $layerdb, $attributenames, $all_languages = false, $recursive = false){
	...
	*/
	function get_datatypes($layer_ids) {
		...
		$query = mysql_query($sql);
		if ($query == 0) {
			$this->GUI->add_message('error', sql_err_msg($PHP_SELF, __LINE__, $sql));
			return 0;
		}
		while ($rs = mysql_fetch_assoc($query)) {
			$datatypes[] = $rs;
