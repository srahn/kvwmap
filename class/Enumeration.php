...
class Enumeration extends MyObject {
	...
	// 	return array(
	// 		'select' => ($attr['is_array'] == 't' ? "CASE WHEN array_length(" . $layer->get_table_alias() . "." . $attr['att_name'] . ", 1) = 0 THEN NULL ELSE array_to_string(" . $layer->get_table_alias() . "." . $attr['att_name'] . ", ',', '') END AS " . $attr['att_name'] : $layer->get_table_alias() . "." . $attr['att_name']) . ",
	...
	function get_generic_select($table_alias, $attr) {
		$selects = array(
			array(
				...
				'sql' => ($attr['is_array' ] == 't' ? "CASE WHEN array_length(" . $table_alias . "." . $attr['att_name'] . ", 1) = 0 THEN NULL ELSE array_to_string(" . $table_alias . "." . $attr['att_name'] . ", ',', '') END AS " . $attr['att_name'] : $selects[] =  $table_alias . "." . $attr['att_name'])
			),
			array(
				'att_name' => $attr['att_name'] . '_text',
				'sql' => "gdi_enum_json_to_text(to_json(" . $table_alias . "." . $attr['att_name'] . "), 'xplan_gml', '" .  $attr['typname'] . "', " . ($attr['is_array'] == 't' ? "true" : "false") . ") AS " . $attr['att_name'] . "_text"
			)
		);
		return $selects;
	}
