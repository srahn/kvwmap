...
#        $this->user->rolle->setSelectedButton('next');
    ...
    $this->debug->write("<p>file:users.php class:rolle->setSelectedButton - Speichern des zuletzt gewÃ¤hlten Buttons aus dem Kartenfensters:",4);
    ...
    if (LOG_CONSUME_ACTIVITY==1) {
      # function setzt eine VerbraucheraktivitÃ¤t (den Zugriff auf Layer oder Daten)
      # Starten der Transaktion
      $sql ='START TRANSACTION';
      $ret=$this->database->execSQL($sql,4, 1);
      if ($ret[0]) {
        # Fehler bei Datenbankanfrage
        $ret[1]='<br>Die Transaktion zur Eintragung der VerbraucheraktivitÃ¤t konnte gestartet werden.<br>'.$ret[1];
      }
      else {
        ...
        if ($ret[0]) {
          ...
          for ($i=0;$i<count($layer);$i++) {
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # Hier eventuell spÃ¤ter mal einbauen, dass geprÃ¼ft wird, ob die Layer wirklich verfÃ¼gbar sind.
            # Wichtig wird das besonders fÃ¼r externe Datenquellen wie fremde WMS oder WFS Layer
            # Die dÃ¼rfen nicht mit abgerechnet werden, wenn sie beim Client nicht erscheinen.
            ...
            if ($ret[0]) {
              # Fehler bei Datenbankanfrage
              $errmsg.='<br>Die VerbraucheraktivitÃ¤t fÃ¼r den Zugiff auf den Layer: '.$layer[$i].' konnte nicht eingetragen werden.<br>'.$ret[1];
            }
      ...
      if ($errmsg!='') {
        ...
        if ($ret[0]) {
          # Fehler bei Datenbankanfrage
          $errmsg.='<br>Die Transaktion zum Eintragen der VerbraucheraktivitÃ¤t konnte nicht abgebrochen werden.<br>'.$ret[1];
        }
        $ret[0]=1;
        $ret[1]=$errmsg;
      }
      else {
        # Es sind keine Fehler innerhalb der Transaktion aufgetreten, Erfolgreich abschlieÃŸen der Transaktion
        $sql ='COMMIT';
        $ret=$this->database->execSQL($sql,4, 1);
        if ($ret[0]) {
          # Fehler bei Datenbankanfrage
          $errmsg.='<br>Die Transaktion zum Eintragen der VerbraucheraktivitÃ¤t konnte nicht erfolgreich abgeschlossen werden.<br>'.$ret[1];
        }
        $ret[0]=0;
        $ret[1]='<br>VerbraucheraktivitÃ¤t erfolgreich eingetragen.';
      }
    }
    else {
      $ret[0]=0;
    ...
    #$this->debug->write("<br>Datenbankverbindung Ã¶ffnen: Datenbank: ".$this->dbName." User: ".$this->user,4);
    ...
    if (DBWRITE OR (!stristr($sql,'INSERT') AND !stristr($sql,'UPDATE') AND !stristr($sql,'DELETE'))) {
      ...
      if ($query==0) {
        ...
        if ($logsql) {
      	...
      	# Abfrage wurde erfolgreich ausgefÃ¼hrt
        $ret[0]=0;
        $ret[1]=$query;
        $this->debug->write("<br>".$sql,$debuglevel);
      ...
      # Es werden keine SQL-Kommandos ausgefÃ¼hrt
      # Die Funktion liefert ret[0]=0, und zeigt damit an, daÃŸ kein Datenbankfehler aufgetreten ist,
      $ret[0]=0;
      ...
      # zusÃ¤tzlich immer in die debugdatei
      # 2006-07-04 pk $logfile ersetzt durch $this->logfile
      if ($logsql) {
        $this->logfile->write($sql.';');
      }
    ...
    # Wenn zu unterstÃ¼tzende SRIDs angegeben sind, ist die Abfrage diesbezÃ¼glich eingeschrÃ¤nkt
    ...
    $this->debug->write("<br>PostgreSQL Verbindung mit ID: ".$this->dbConn." schlieÃŸen.",4);
