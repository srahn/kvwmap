...
TRUNCATE alkis.delete;

DROP TRIGGER delete_feature_trigger ON alkis.delete;

DROP FUNCTION alkis.delete_feature_hist();
...
BEGIN
	...
	IF (length(NEW.featureid)=32) THEN
		NEW.featureid = substr(NEW.featureid, 1, 16);
	...
	ELSIF NEW.context='replace' THEN
		...
		IF NEW.endet IS NULL THEN
			EXECUTE 'SELECT max(beginnt) FROM ' || NEW.typename || ' a'
...
BEGIN
	...
	LOOP
		...
		END IF;
		s := s || ' WHERE gml_id=''' || r.featureid || '''' || ' AND endet IS NULL AND beginnt < ''' || r.endet || '''';
		EXECUTE s;
		RAISE NOTICE '%', s;
