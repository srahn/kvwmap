...
UPDATE alkis.pp_flurstueckshistorie t
SET vorgaengerflurstueckskennzeichen = sub.vorgaenger
FROM (
    SELECT 
        t2.flurstueckskennzeichen,
        array_agg(t1.flurstueckskennzeichen) AS vorgaenger
    FROM alkis.pp_flurstueckshistorie t1
    JOIN alkis.pp_flurstueckshistorie t2
        ON t2.flurstueckskennzeichen = ANY(t1.nachfolgerflurstueckskennzeichen)
    GROUP BY t2.flurstueckskennzeichen
) sub
WHERE t.flurstueckskennzeichen = sub.flurstueckskennzeichen
  AND t.vorgaengerflurstueckskennzeichen IS NULL;
