...
-- FUNCTION: alkis.postprocessing()

-- DROP FUNCTION IF EXISTS alkis.postprocessing();
...
INSERT INTO 
	...
	SELECT  
		v.land, g.regierungsbezirk, v.kreis, v.verwaltungsgemeinschaft,
	...
	where 
		...
		v.endet is null 
	GROUP BY
		v.land, g.regierungsbezirk, v.kreis, v.verwaltungsgemeinschaft, v.bezeichnung
  	ORDER BY        v.land, g.regierungsbezirk, v.kreis, v.verwaltungsgemeinschaft;
...
INSERT INTO 
	...
	SELECT  
		g.land, g.regierungsbezirk, g.kreis, schluesselgesamt as amt,
		bezeichnung as amt_name,
		1,
		pp.the_geom
  	FROM
		alkis.ax_gemeinde g
...
AND k.endet IS NULL
GROUP BY a.land, a.regierungsbezirk, a.kreis, k.bezeichnung order by bezeichnung;
...
-- -------------------------------------
INSERT INTO alkis.n_nutzung (gml_id, beginnt, endet, objektart, nutzungsartengruppe,            werteart1,            werteart2, info, 				 zustand, name, bezeichnung, wkb_geometry)
  SELECT                     gml_id, beginnt, endet,     43002,                  32, coalesce(nutzung, 0), coalesce(zustand, 0), vegetationsmerkmal, null, name, bezeichnung, wkb_geometry 
  FROM alkis.ax_wald
  WHERE st_geometrytype(wkb_geometry) = 'ST_Polygon';
...
truncate alkis.pp_aktualitaet;

insert into alkis.pp_aktualitaet
	select max(beginnt) from alkis.ax_flurstueck;
