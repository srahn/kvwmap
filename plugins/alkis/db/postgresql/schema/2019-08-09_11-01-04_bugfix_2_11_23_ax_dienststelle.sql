...
CREATE OR REPLACE VIEW alkis.lk_ap AS 
 SELECT 
   ...
   FROM alkis.ax_aufnahmepunkt o
     ...
     LEFT JOIN alkis.ax_dienststelle d ON o.zustaendigestelle_land = d.land AND o.zustaendigestelle_stelle::text = d.stelle::text
...
CREATE OR REPLACE VIEW alkis.lk_grenzpunkte AS 
 SELECT 
   ...
   FROM alkis.ax_grenzpunkt o
     ...
     LEFT JOIN alkis.ax_datenerhebung_punktort e ON e.wert = ANY (p.processstep_ax_datenerhebung_punktort::integer[])
     LEFT JOIN alkis.ax_dienststelle d ON o.zustaendigestelle_land = d.land AND o.zustaendigestelle_stelle::text = d.stelle::text
     LEFT JOIN alkis.ax_dienststelle da ON o.zustaendigestelle_land = da.land AND o.ausgesetzteabmarkung_stelle::text = da.stelle::text
     LEFT JOIN alkis.ax_koordinatenstatus_punktort ks ON p.koordinatenstatus = ks.wert
