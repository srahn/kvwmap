...
BEGIN
	...
	LOOP
		s := 'UPDATE alkis.' || r.typename || ' SET endet = ''' || coalesce(r.endet, substr(r.replacedby, 17)) || '''';
		IF r.context='update' AND r.anlass IS NOT NULL THEN
			s := s || ',anlass=array_cat(anlass,''{' || array_to_string(r.anlass,',') || '}'')';
		END IF;
		s := s || ' WHERE gml_id=''' || r.featureid || '''' || ' AND beginnt < ''' || coalesce(r.endet, substr(r.replacedby, 17)) || ''' AND (endet IS NULL OR endet = ''' || coalesce(r.endet, substr(r.replacedby, 17)) || ''')';
		EXECUTE s;
		--RAISE INFO '%', s;
