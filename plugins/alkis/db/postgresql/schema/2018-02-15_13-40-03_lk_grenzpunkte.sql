...
CREATE OR REPLACE VIEW alkis.lk_grenzpunkte AS 
 SELECT COALESCE(p.oid, o.oid) AS oid,
    ...
    p.processstep_ax_datenerhebung_punktort[0] AS source_description,
   ...
   FROM alkis.ax_grenzpunkt o
     ...
     LEFT JOIN alkis.ax_datenerhebung e ON e.wert = ANY (p.herkunft_source_source_ax_datenerhebung_punktort::integer[])
