...
CREATE OR REPLACE RULE r1_auftragsdatei_pruefen AS
	...
	WHERE ( SELECT new.auftragsdatei != '' AND new.antragsnr::text <> substring(split_part(new.auftragsdatei::text, 'original_name='::text, 2) from 1 for length(new.antragsnr::text))) DO INSTEAD  SELECT 'Die Antragsnummer im Dateinamen entspricht nicht der Antragsnummer im Formular oben.', 'error' AS msg_type;
