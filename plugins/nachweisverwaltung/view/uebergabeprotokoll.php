...
function create_protocol(){
	...
	document.GUI.go.value = 'Antraganzeige_Uebergabeprotokoll_Erzeugen';
	document.GUI.submit();
	document.GUI.target = '';
...
<? 		foreach($this->hauptarten as $hauptart){ ?>
          <td width="50px" <? if(!in_array($hauptart['id'], $this->formvars['hauptart'])) echo 'bgcolor="#EBEBEB"'; ?>><span class="fett"><? echo $hauptart['abkuerzung']; ?></span><input type="checkbox" onchange="document.GUI.submit();" value="<? echo $hauptart['id']; ?>" name="hauptart[]" <? if(in_array($hauptart['id'], $this->formvars['hauptart']))echo 'checked=true;'; ?>></td>
<? 		} ?>
...
<? 		foreach($this->hauptarten as $hauptart){ ?>					
          <td valign="top" align="center" <? if(!in_array($hauptart['id'], $this->formvars['hauptart'])) echo 'bgcolor="#EBEBEB">';else echo '>'.$this->antrag->FFR[$i][$hauptart['abkuerzung']]; ?></td>
<? 		} ?>
  ...
  <tr> 
    <td bgcolor="<? echo BG_FORM ?>">
      ...
      <input type="hidden" name="go" value="Antraganzeige_Uebergabeprotokoll_Zusammenstellen">
