<?
	...
	$this->packAndMail = function($antrag_id, $data) use ($GUI) {
		...
		$success = mail_att($mail_bearbeiter['from_name'], $mail_bearbeiter['from_email'], $mail_bearbeiter['to_email'], $mail_bearbeiter['cc_email'], $mail_bearbeiter['reply_email'],	$mail_bearbeiter['subject'], $mail_bearbeiter['message'], $mail_bearbeiter['attachement']);
		
		if ($success and $data['email'] != '') {
			# create and send email for absender
			$mail_absender = array();
			include (PLUGINS . $plugin_name . '/view/email_absender_template.php');
			$success = mail_att($mail_absender['from_name'], $mail_absender['from_email'], $mail_absender['to_email'], $mail_absender['cc_email'], $mail_absender['reply_email'],	$mail_absender['subject'], $mail_absender['message'], $mail_absender['attachement']);
		}
