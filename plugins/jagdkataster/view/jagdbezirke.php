...
<?php
	...
	################################################################	
	for ($j=0;$j<$anzObj;$j++) {
		$paechterliste = $jagdkataster->get_paechter($this->qlayerset[$i]['shape'][$j]['gid']);
?>
  ...
  <tr>
    <td>
			<input type="hidden" value="1" name="changed_<? echo $this->qlayerset[$i]['Layer_ID'].'_'.$this->qlayerset[$i]['shape'][$j]['gid']; ?>">
      ...
      <colgroup>
        <tr width="100%">
          ...
          <td valign="top">
          	<input type="text" <? if($privileg_['name'] == 0)echo 'readonly'; ?> name="<? echo $this->qlayerset[$i]['Layer_ID'].';name;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1'; ?>" value="<? echo $this->qlayerset[$i]['shape'][$j]['name']; ?>">
          	<?
          	$this->form_field_names .= $this->qlayerset[$i]['Layer_ID'].';name;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1|';
          	?>
          </td>
        </tr>
        ...
        <tr width="100%">
          ...
          <td valign="top">
          	<select onchange="update_form(this.value);" name="<? echo $this->qlayerset[$i]['Layer_ID'].';art;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1'; ?>">
              ...
              <option <? if($this->qlayerset[$i]['shape'][$j]['art'] == 'ajb'){echo 'selected';} ?> value="ajb">Abgerundeter Eigenjagdbezirk</option>
			  		...
			  		<?
			  		$this->form_field_names .= $this->qlayerset[$i]['Layer_ID'].';art;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1|';
			  		?>
          </td>
        </tr>
        ...
        <tr id="lfdnr" width="100%" style="display:<? if(in_array($this->qlayerset[$i]['shape'][$j]['art'], array('jbe', 'jbf', 'agf', 'atf', 'apf', 'atv'))){ echo 'none';}else{echo '';} ?>">
          ...
          <td valign="top">
          	<input type="text" name="<? echo $this->qlayerset[$i]['Layer_ID'].';id;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1'; ?>" value="<? echo $this->qlayerset[$i]['shape'][$j]['id']; ?>">
          	<?
			  		$this->form_field_names .= $this->qlayerset[$i]['Layer_ID'].';id;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1|';
			  		?>
          </td>
        </tr>
        ...
        <tr id="zuordnung" width="100%" style="display:<? if(in_array($this->qlayerset[$i]['shape'][$j]['art'], array('jbe', 'jbf', 'agf', 'atf', 'apf', 'atv'))){ echo '';}else{echo 'none';} ?>">
          ...
          <td valign="top">
          	<input type="text" name="<? echo $this->qlayerset[$i]['Layer_ID'].';jb_zuordnung;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1'; ?>" value="<? echo $this->qlayerset[$i]['shape'][$j]['jb_zuordnung']; ?>">
          	<?
			  		$this->form_field_names .= $this->qlayerset[$i]['Layer_ID'].';jb_zuordnung;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1|';
			  		?>
          </td>
        </tr>
        <tr id="status" width="100%" style="display:<? if(in_array($this->qlayerset[$i]['shape'][$j]['art'], array('ejb', 'jbe', 'jbf', 'agf', 'atf', 'apf', 'atv'))){ echo '';}else{echo 'none';} ?>">
          ...
          <td valign="top">
          	<select name="<? echo $this->qlayerset[$i]['Layer_ID'].';status;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1'; ?>">
          	...
          	<?
			  		$this->form_field_names .= $this->qlayerset[$i]['Layer_ID'].';status;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1|';
			  		?>
          </td>
        </tr>
        ...
        <tr id="verzicht" width="100%" style="display:<? if(in_array($this->qlayerset[$i]['shape'][$j]['art'], array('ejb','ajb'))){ echo '';}else{echo 'none';} ?>">
          ...
          <td valign="top">
          	<select name="<? echo $this->qlayerset[$i]['Layer_ID'].';verzicht;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1'; ?>">
          	...
          	<?
			  		$this->form_field_names .= $this->qlayerset[$i]['Layer_ID'].';verzicht;jagdbezirke;'.$this->qlayerset[$i]['shape'][$j]['gid'].';Text;1|';
			  		?>
          </td>
        </tr>
        ...
        <tr width="100%">
          <td valign="top" bgcolor="<?php echo BG_DEFAULT ?>">
            ...
            ?>
            <a href="javascript:paechter_listen(<?php echo $this->qlayerset[$i]['shape'][$j]['gid']; ?>, '<? echo $this->qlayerset[$i]['shape'][$j]['name'] ?>');"><span class="fett">P&auml;chter</span></a>&nbsp;
            <?php
            }
            ?>
          </td>
  ...
  &nbsp; <!-- hier muss noch die Anzeige der sowie der Link zur Abschussplanung hin! -->
        ...
        <tr>
          <td colspan="2" bgcolor="<?php echo BG_DEFAULT ?>">
            <a target="root" style="font-size: <? echo $this->user->rolle->fontsize_gle; ?>px" href="" onclick="this.href='index.php?go=zoomtoPolygon&oid=<?php echo $this->qlayerset[$i]['shape'][$j]['gid']; ?>&layer_tablename=jagdbezirke&layer_columnname=the_geom&layer_id=<? echo $this->qlayerset[$i]['Layer_ID'];?>&selektieren='+currentform.selektieren<? echo $this->qlayerset[$i]['Layer_ID'].'_'.$j; ?>.checked;">Kartenausschnitt</a>&nbsp;&nbsp;<span style="font-size: <? echo $this->user->rolle->fontsize_gle; ?>px">Selektieren</span><input type="checkbox" name="selektieren<? echo $this->qlayerset[$i]['Layer_ID'].'_'.$j; ?>" value="1">&nbsp;|&nbsp;
						<? if($flst_privileges['eigentuemer'] != ''){ ?>
            <a target="root" href="index.php?go=jagdkatastereditor_Flurstuecke_Listen&oid=<?php echo $this->qlayerset[$i]['shape'][$j]['gid']; ?>&name=<? echo $this->qlayerset[$i]['shape'][$j]['name'] ?>&search_nummer=<? echo $this->formvars['search_nummer']; ?>&search_name=<? echo $this->formvars['search_name']; ?>&search_art=<? echo $this->formvars['search_art']; ?>">enthaltene Flurstücke</a>&nbsp;|&nbsp;
            <a target="root" href="index.php?go=jagdkatastereditor_Eigentuemer_Listen&oid=<?php echo $this->qlayerset[$i]['shape'][$j]['gid']; ?>&name=<? echo $this->qlayerset[$i]['shape'][$j]['name'] ?>&search_nummer=<? echo $this->formvars['search_nummer']; ?>&search_name=<? echo $this->formvars['search_name']; ?>&search_art=<? echo $this->formvars['search_art']; ?>">Eigentümer auflisten</a>&nbsp;|&nbsp;
						<? }
							 if($editable){ ?>
            <a target="root" href="index.php?go=jagdkatastereditor&oid=<?php echo $this->qlayerset[$i]['shape'][$j]['gid']; ?>">bearbeiten</a>&nbsp;|&nbsp;
            <a target="root" href="javascript:Bestaetigung('index.php?go=jagdkatastereditor_Loeschen&oid=<?php echo $this->qlayerset[$i]['shape'][$j]['gid']; ?>', 'Wollen Sie diesen Jagdbezirk wirklich löschen?');">löschen</a>&nbsp;|&nbsp;
            <a target="root" href="index.php?go=jagdkatastereditor_kopieren&oid=<?php echo $this->qlayerset[$i]['shape'][$j]['gid']; ?>">kopieren</a>
						<? } ?>
          </td>
        </tr>
				<? if($editable){ ?>
...
<input name="value_oid" type="hidden" value="<? echo $this->qlayerset[$i]['shape'][0]['gid']; ?>">
