  ...
  &nbsp; <!-- hier muss noch die Anzeige der sowie der Link zur Abschussplanung hin! -->
        ...
        <tr>
          <td colspan="2" bgcolor="<?php echo BG_DEFAULT ?>">
            <a target="root" style="font-size: <? echo $this->user->rolle->fontsize_gle; ?>px" href="" onclick="this.href='index.php?go=zoomto_dataset&oid=<?php echo $this->qlayerset[$i]['shape'][$j][$this->qlayerset[$i]['maintable'] . '_oid']; ?>&layer_tablename=jagdbezirke&layer_columnname=the_geom&layer_id=<? echo $this->qlayerset[$i]['Layer_ID'];?>&selektieren='+currentform.selektieren<? echo $this->qlayerset[$i]['Layer_ID'].'_'.$j; ?>.checked;">Kartenausschnitt</a>&nbsp;&nbsp;<span style="font-size: <? echo $this->user->rolle->fontsize_gle; ?>px">Selektieren</span><input type="checkbox" name="selektieren<? echo $this->qlayerset[$i]['Layer_ID'].'_'.$j; ?>" value="1">&nbsp;|&nbsp;
