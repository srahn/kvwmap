...
CREATE OR REPLACE VIEW jagdkataster.befriedete_bezirke_flurstuecke AS 
 SELECT b.id,
   ...
   FROM jagdkataster.befriedete_bezirke b
     LEFT JOIN alkis_alt.ax_flurstueck a ON st_intersects(b.the_geom, a.wkb_geometry) AND a.endet IS NULL AND (st_area(st_intersection(b.the_geom, a.wkb_geometry)) / st_area(a.wkb_geometry) * 100::numeric) > 0.1
     LEFT JOIN alkis_alt.n_nutzung n ON st_intersects(n.wkb_geometry, a.wkb_geometry) AND st_area(st_intersection(n.wkb_geometry, a.wkb_geometry)) > 0.001 AND st_area(st_intersection(b.the_geom, st_intersection(n.wkb_geometry, a.wkb_geometry))) > 0.1
     LEFT JOIN alkis_alt.n_nutzungsartenschluessel nas ON n.nutzungsartengruppe = nas.nutzungsartengruppe AND n.werteart1 = nas.werteart1 AND n.werteart2 = nas.werteart2
     LEFT JOIN alkis_alt.n_nutzungsartengruppe nag ON nas.nutzungsartengruppe = nag.schluessel
