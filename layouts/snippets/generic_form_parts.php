<?
	...
	function attribute_value(&$gui, $layer_id, $attributes, $j, $k, $dataset, $size, $select_width, $fontsize, $change_all = false){
		...
		if($attributes['constraints'][$j] != '' AND !in_array($attributes['constraints'][$j], array('PRIMARY KEY', 'UNIQUE'))){
			...
			switch ($attributes['form_element_type'][$j]){
				case 'Textfeld' : {
					$datapart .= '<textarea title="'.$alias.'" id="'.$name.'_'.$k.'" cols="45" onchange="'.$onchange.'"';
				...
				case 'Auswahlfeld' : case 'Auswahlfeld_not_saveable' : {
					...
					$datapart .= Auswahlfeld($layer_id, $name, $j, $alias, $fieldname, $value, $enum_value, $enum_output, $attributes['req_by'][$j], $attributes['name'], $attribute_privileg, $k, $oid, $attributes['subform_layer_id'][$j], $attributes['subform_layer_privileg'][$j], $attributes['embedded'][$j], $lock[$k], $select_width, $fontsize, $strPleaseSelect, $onchange);
				}break;
					...
					for($f = 0; $f < count($attribute_foreign_keys); $f++){											
						...
						switch ($attributes['form_element_type'][$attribute_foreign_keys[$f]]){
							...
							case 'Auswahlfeld' : {
								...
								$datapart .= Auswahlfeld($layer_id, $name_, $j, $attributes['alias'][$name_], $fieldname_[$f], $dataset[$name_], $enum_value, $enum_output, $attributes['req_by'][$index], $attributes['name'], $attributes['privileg'][$name_], $k, $oid, $attributes['subform_layer_id'][$index], $attributes['subform_layer_privileg'][$index], $attributes['embedded'][$index], $lock[$k], $select_width, $fontsize, $strPleaseSelect, $onchange);
	...
	function Auswahlfeld($layer_id, $name, $j, $alias, $fieldname, $value, $enum_value, $enum_output, $req_by, $attributenames, $privileg, $k, $oid, $subform_layer_id, $subform_layer_privileg, $embedded, $lock, $select_width, $fontsize, $strPleaseSelect, $onchange){
		if($privileg == '0' OR $lock){
