			...
			switch ($attributes['form_element_type'][$j]){
				case 'Textfeld' : {
					...
					if($attribute_privileg == '0' OR $lock[$k]){ // nur lesbares Attribut
						if($size == 12){		// spaltenweise
							...
							$maxwidth = $size * 11;
							$minwidth = $size * 7.1;
							$datapart .= '<div style="padding: 0 0 0 3; min-width: '.$minwidth.'px; max-width:'.$maxwidth.'px; font-size: '.$fontsize.'px;"><pre>' . htmlspecialchars($value) . '</pre></div>';
						}
					}
				}break;
				...
				case 'Radiobutton' : {
					...
					for($e = 0; $e < count($enum_value); $e++){
						...
						if($attributes['nullable'][$j] != '0')$datapart .= ' onclick="'.($attribute_privileg == '0'? 'return false;' : '').'this.checked = this.checked2; if(this.checked===false){var evt = document.createEvent(\'HTMLEvents\');evt.initEvent(\'change\', false, true); this.dispatchEvent(evt);}" onmousedown="this.checked2 = !this.checked;"';							
