		...
		if($attributes['constraints'][$j] != '' AND !in_array($attributes['constraints'][$j], array('PRIMARY KEY', 'UNIQUE'))){
			...
			switch ($attributes['form_element_type'][$j]){
				...
				case 'Dokument': {
					...
					if($attribute_privileg != '0' AND !$lock[$k]){
						$datapart .= '<input onchange="'.$onchange.'" style="font-size: '.$fontsize.'px" size="43" type="file" onchange="this.title=this.value;" id="'.$layer_id.'_'.$name.'_'.$k.'" name="'.$fieldname.'">';
					}
					else{
						$datapart .= '&nbsp;';
					}
				...
				case 'Link': {
					...
					if($attribute_privileg != '0' OR $lock[$k]){
						$datapart .= '<input onchange="'.$onchange.'" id="'.$layer_id.'_'.$name.'_'.$k.'" style="font-size: '.$fontsize.'px" size="'.$size.'" type="text" name="'.$fieldname.'" value="'.htmlspecialchars($value).'">';
					}else{
