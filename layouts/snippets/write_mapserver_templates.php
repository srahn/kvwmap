...
if ($this->formvars['go_plus'] == '') { ?>
	...
	<tr><th>Layer</th><th>write_mapserver_templates</th></tr><?php
	foreach($this->layers AS $layer) { ?>
		<tr><td style="text-align: left"><? echo $layer->get('Name'); ?></td><td><? echo $write_options[$layer->get('write_mapserver_templates')]; ?></td></tr><?
	} ?>
	</table>
	<input type="checkbox" value="Tabelle" name="ansicht" checked/> Darstellung im Template in Tabellenform
	<input type="submit" value="Erzeugen" name="go_plus" style="margin-top: 20px; margin-left: 5px; margin-bottom: 20px"/>
	...
	<table border="1" cellpadding="2" cellspacing="0" style="margin-top: 20px">
		...
		foreach($this->layers AS $layer) { ?>
			<tr><td style="text-align: left"><? echo $layer->get('Name'); ?></td><td style="text-align: center"><i class="fa fa-check" style="color: darkgreen"></i></td></tr><?
		} ?>
	</table>
	<p><p><?
} ?>