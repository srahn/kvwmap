...
<?
	...
	$executiontimes['action'][] = 'attr start';
	for ($k=0;$k<$anzObj;$k++) {
		$datapart = '';
			      ...
			      <td align="right">
							<table cellspacing="0" cellpadding="0">
								<tr>
									...
									<? } ?>
			     	<? 	if($layer['privileg'] > '0'){ ?>
									<td style="padding: 0 0 0 10;"><a href="javascript:select_this_dataset(<? echo $layer['Layer_ID']; ?>, <? echo $k; ?>);use_for_new_dataset(<? echo $layer['Layer_ID']; ?>, <? echo $k; ?>)" title="<? echo $strUseForNewDataset; ?>"><div class="button_background"><div class="emboss use_for_dataset"><img  src="<? echo GRAPHICSPATH.'leer.gif'; ?>"></div></div></a></td>
			      <? 	} 
			...
			for($j = 0; $j < count($attributes['name']); $j++){
				...
				if($attributes['invisible'][$attributes['name'][$j]] != 'true' AND $attributes['name'][$j] != 'lock'){
					if($attributes['type'][$j] != 'geometry'){
							...
							$datapart .= attribute_name($attributes, $j, $this->user->rolle->fontsize_gle);
...
<?
					...
					$executiontimes['time'][] = microtime_float1();
				$executiontimes['action'][] = 'attr ende';
				...
				<tr>
					<td>
						<table cellspacing="0" cellpadding="0">
							<tr>
						<? if($this->formvars['go'] == 'Zwischenablage' OR $this->formvars['go'] == 'gemerkte_Datensaetze_anzeigen'){ ?>
								<td style="padding: 5 10 0 0;"><a title="<? echo $strDontRememberDataset; ?>" href="javascript:select_this_dataset(<? echo $layer['Layer_ID']; ?>, <? echo $k; ?>);remove_from_clipboard(<? echo $layer['Layer_ID']; ?>);"><div class="button_background"><div class="emboss nicht_mehr_merken"><img  src="<? echo GRAPHICSPATH.'leer.gif'; ?>"></div></div></a></td>
							<? }else{ ?>
