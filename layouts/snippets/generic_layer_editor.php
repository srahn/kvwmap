						...
						for($j = 0; $j < count($this->qlayerset[$i]['attributes']['name']); $j++){
							...
							if($j == 0 OR $layer['attributes']['group'][$j] != $layer['attributes']['group'][$j+1]){
								...
								if($j > 0){
									...
									echo '&nbsp;<a href="javascript:void(0);" onclick="toggleGroup(\''.$groupname.'\')"><img id="img_'.$groupname.'" border="0" src="graphics/'.($collapsed ? 'plus' : 'minus').'.gif"></a>&nbsp;'.$groupname.'</td><td style="border:none;background: url(graphics/bg.gif);"></td>';
									$colspan = 0;
								}
								else $colspan = 1;
