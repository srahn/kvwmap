...
<?		$trans_oid = explode('|', $this->qlayerset[$i]['shape'][$k]['lock']);
			if($trans_oid[1] != '' AND $this->qlayerset[$i]['shape'][$k][$this->qlayerset[$i]['attributes']['table_name'][$this->qlayerset[$i]['attributes']['name'][0]].'_oid'] == $trans_oid[1]){
				echo '<tr><td colspan="0" align="center"><span class="red">Dieser Datensatz ist gelockt und kann nicht bearbeitet werden.</span></td></tr>';
				$lock[$k] = true;
			}
                      ...
                      $auswahlfeld_output_laenge = '';
										...
										if($this->new_entry != true AND $no_query != true){
											...
											if($this->qlayerset[$i]['attributes']['subform_layer_privileg'][$j] > 0 AND !$lock[$k]){
												if($this->qlayerset[$i]['attributes']['embedded'][$j] == true){
													...
													$data .= '&preview_attribute='.$this->qlayerset[$i]['attributes']['preview_attribute'][$j];
													echo '&data='.str_replace('&', '<und>', $data);
													echo '&selected_layer_id='.$this->qlayerset[$i]['attributes']['subform_layer_id'][$j].'&embedded=true&fromobject=subform'.$this->qlayerset[$i]['Layer_ID'].'_'.$k.'_'.$j.'&targetobject='.$this->qlayerset[$i]['attributes']['name'][$j].'_'.$k.'&targetlayer_id='.$this->qlayerset[$i]['Layer_ID'].'&targetattribute='.$this->qlayerset[$i]['attributes']['name'][$j].'\', new Array(document.getElementById(\'subform'.$this->qlayerset[$i]['Layer_ID'].'_'.$k.'_'.$j.'\')), new Array(\'sethtml\'));clearsubforms();">&nbsp;<? echo $strNewEmbeddedPK; ?></a></td></tr></table>';
													echo '<div style="display:inline" id="subform'.$this->qlayerset[$i]['Layer_ID'].'_'.$k.'_'.$j.'"></div>';
												}
												else{
									...
									case 'dynamicLink': {
										...
										if($explosion[2] == 'embedded'){
											echo '<a href="javascript:if(document.getElementById(\'dynamicLink'.$this->qlayerset[$i]['Layer_ID'].'_'.$k.'_'.$j.'\').innerHTML != \'\'){clearsubform(\'dynamicLink'.$this->qlayerset[$i]['Layer_ID'].'_'.$k.'_'.$j.'\');} else {ahah(\''.$href.'\', \'\', new Array(document.getElementById(\'dynamicLink'.$this->qlayerset[$i]['Layer_ID'].'_'.$k.'_'.$j.'\')), new Array(\'sethtml\'))}">';
											echo $alias;
											echo '</a><br>';
...
?>
							<br><br>&nbsp;&nbsp;
							<a style="font-size: <? echo $this->user->rolle->fontsize_gle; ?>px" href="index.php?go=Bodenrichtwertformular_Aendern&layer_id=<? echo $this->qlayerset[$i]['Layer_ID']; ?>&oid=<?php echo $this->qlayerset[$i]['shape'][$k][$tablename.'_oid']; ?>">Richtwert &auml;ndern</a></td>
			    </tr>
<? }
