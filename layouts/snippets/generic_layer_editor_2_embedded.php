				...
				if($layer['attributes']['visible'][$j] AND ($this->new_entry != true OR $layer['attributes']['dont_use_for_new'][$j] != -1)){
					if($layer['attributes']['type'][$j] != 'geometry'){
						if($layer['attributes']['SubFormFK_hidden'][$j] != 1){
							...
							$nl = ($layer['attributes']['labeling'][$j] == 1); # Attributname soll oben stehen -> alle weiteren tds für die nächste Zeile aufsammeln
							$td = '	<td width="" id="value_'.$layer['layer_id'].'_'.$layer['attributes']['name'][$j].'_'.$k.'" ' . get_td_class_or_style(array(
								$layer['shape'][$k][$layer['attributes']['style_attribute'][$j]],
								'gle_attribute_value value_'.$layer['layer_id'].'_'.$layer['attributes']['name'][$j]
							)) . ($layer['attributes']['arrangement'][$j+1] != 1 ? 'colspan="20"' : '') . '>'
							. attribute_value($this, $layer, NULL, $j, $k, NULL, $size, $select_width, false, NULL, NULL, NULL, $this->subform_classname)
							. '	</td>';
...
?>								
	...
	<?								} 
									if($layer['shape'][$k][$layer['attributes']['the_geom']]){ ?>
											<td style="padding: 0 0 0 10;"><a title="<? echo $strMapZoom; ?>" href="javascript:zoom2object(<? echo ($layer['collection_layer_layer_id'] ?: $layer['layer_id']);?>, '<? echo $columnname; ?>', '<? echo $layer['shape'][$k][$layer['maintable'].'_oid']; ?>', 'zoomonly');"><div class="button zoom_normal"><img src="<? echo GRAPHICSPATH.'leer.gif'; ?>"></div></a></td>
									<? if($layer['layer_id'] > 0){ ?>
											<td style="padding: 0 0 0 10;"><a title="<? echo $strMapZoom.$strAndHighlight; ?>" href="javascript:zoom2object(<? echo ($layer['collection_layer_layer_id'] ?: $layer['layer_id']);?>, '<? echo $columnname; ?>', '<? echo $layer['shape'][$k][$layer['maintable'].'_oid']; ?>', 'false');"><div class="button zoom_highlight"><img src="<? echo GRAPHICSPATH.'leer.gif'; ?>"></div></a></td>
											<td style="padding: 0 0 0 10;"><a title="<? echo $strMapZoom.$strAndHide; ?>" href="javascript:zoom2object(<? echo ($layer['collection_layer_layer_id'] ?: $layer['layer_id']);?>, '<? echo $columnname; ?>', '<? echo $layer['shape'][$k][$layer['maintable'].'_oid']; ?>', 'true');"><div class="button zoom_select"><img src="<? echo GRAPHICSPATH.'leer.gif'; ?>"></div></a></td>
									<? }} ?>
										</tr>
									</table>
								</td>
