...
#	include('../../config.php');
	...
	# ToDo pk: Später ersetzen durch richtige Konstanen in config
...
?>
input {
	font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em
}
#form-titel {
...
.form_formular-input > div:first-child, .form_formular-input-selector > div:first-child {
	...
	font-family: SourceSansPro3;
	line-height: 20px;
	font-size: <? echo FONT_SIZE_FACTOR * 17; ?>em;
}
...
input[type="button"][value='«'], input[type="button"][value='»'] {
	...
	font-size: <? echo FONT_SIZE_FACTOR * 24; ?>em;
	height: 35px;
}
...
.px13 {
	font-size: <? echo FONT_SIZE_FACTOR * 13; ?>em;
}

.px14 {
	font-size: <? echo FONT_SIZE_FACTOR * 14; ?>em;
}
...
.px16{
	font-size: <? echo FONT_SIZE_FACTOR * 16; ?>em;
	line-height: 16px;
}
...
.px20{
	font-size: <? echo FONT_SIZE_FACTOR * 20; ?>em;
}
...
ul{
	...
	margin: 5px;
	padding: 0 0 0 <? echo FONT_SIZE_FACTOR * 15; ?>em;
...
input[type="button"], input[type="reset"], input[type="submit"] {
	...
	font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;
	font-family: SourceSansPro1;
	line-height : <? echo FONT_SIZE_FACTOR * 15; ?>em;
}
...
span[data-tooltip]:hover::after {
  ...
  max-width: var(--width);
	font-size: <? echo FONT_SIZE_FACTOR * 11; ?>em;
	font-family: verdana, arial;
	box-shadow: 12px 10px 14px #777;
  border: 1px #236dbf solid;
	border-top: <? echo FONT_SIZE_FACTOR * 15; ?>em #236dbf solid;
...
.image-select .placeholder.editable::after {
  transform: rotate(-45deg);
  right: <? echo FONT_SIZE_FACTOR * 15; ?>em;
}
...
.input-form label {
	...
	font-size: <? echo FONT_SIZE_FACTOR * 17; ?>em;
	width: 240px;
	height: <? echo FONT_SIZE_FACTOR * 15; ?>em;
	margin-right: 10px;
}
...
#datendrucklayouteditor_formular_scroll{
	...
	width: 100%;
	min-width: 600px;
	margin-right: <? echo FONT_SIZE_FACTOR * 15; ?>em;
}
...
#geo_name_search_result_div ul{
	padding: 10px 0 6px <? echo FONT_SIZE_FACTOR * 15; ?>em;
}
...
.code{
	text-align: left;
	font-family: courier;
	font-size: <? echo FONT_SIZE_FACTOR * 12; ?>em;
	line-height: 17px;
}
...
#data_import_upload_progress{
	...
	border-collapse: separate;
  border-spacing: 5px;
	margin-top: <? echo FONT_SIZE_FACTOR * 15; ?>em;
}
...
#agreement_message{
	...
	position: absolute;
	top: <? echo FONT_SIZE_FACTOR * 15; ?>em;
	left: 50%;
...
#agreement_message_button{
	...
	position: relative;
	text-align: center;
	padding: <? echo FONT_SIZE_FACTOR * 15; ?>em;
}
...
.raster_record .tr_hide input[type=checkbox]{display:none;width:12px;font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;height:12px;transition: all 0.25s ease;}
...
.raster_record .tr_show .readonly_text{font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;min-width: 122px !important;max-width: 122px !important;transition: all 0.25s ease;}
...
.raster_record .tr_show input{width:130px;font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;height:22px;transition: all 0.25s ease;}		/* normale Inputfelder werden auf 130px Breite verkleinert*/
...
.raster_record_open a{font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;transition: all 0.25s ease;}
.raster_record_open input[type=text]{width:200px;font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;height:22px;transition: all 0.25s ease;}
.raster_record_open input[type=checkbox]{width:12px;font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;height:12px;transition: all 0.25s ease;}
.raster_record_open textarea{font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;transition: all 0.25s ease;}
.raster_record_open .tr_show #formelement{width: 100%;overflow: visible}
.raster_record_open .readonly_text{font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;min-width: 400px !important;max-width: 400px !important;transition: all 0.25s ease;}
.raster_record_open .tr_show input[type=file]{width:290px;font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;height:22px;transition: all 0.25s ease;}
.raster_record_open select{font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;display:inline;width:290px;transition: all 0.25s ease;}
.raster_record_open select:focus{font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;display:inline;width:290px;transition: all 0.25s ease;}
.raster_record_open span{line-height:16px;font-size: <? echo FONT_SIZE_FACTOR * 15; ?>em;transition: all 0.25s ease;}
...
.calendar table tfoot td {
	font-size: <? echo FONT_SIZE_FACTOR * 7; ?>em;
	border: none;
}
...
.timepicker{
	min-width: 205px;
	font-size: <? echo FONT_SIZE_FACTOR * 18; ?>em;
	line-height: 24px;
	outline: solid #112A5D 1px;
	padding: 3px 0px;
	margin-top: 3px;
...
.timepicker .submit{
	...
	right: 10px;
	margin-top: 1px;
	font-size: <? echo FONT_SIZE_FACTOR * 19; ?>em;
	color: silver;
}
...
.fa-7x {
	font-size: <? echo FONT_SIZE_FACTOR * 117; ?>em !important;
}
